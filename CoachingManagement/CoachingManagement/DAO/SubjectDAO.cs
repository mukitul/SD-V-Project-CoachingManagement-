    ...
    class SubjectDAO
    {
        ...
        public void AddSubject(SubjectDTO subjectDTO)
        {
            try
            {
                ...
                connectionDB.ExecuteQueries(sqlQuery);
                connectionDB.CloseConnection();
                MessageBox.Show("Subject added successfully to the database", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public void UpdateSubject(string classname,string classcode, string subname, string subcode, int id)
        {
            ...
            catch (SqlException ex)
            {
                if (ex.Number > 0)
                {
                    //Violation of primary key. Handle Exception
                    connectionDB.CloseConnection();
                    MessageBox.Show("Subject Code already exists. \n Try a new code.", "Update Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
