...
namespace CoachingManagement
{
    class StudentDAO
    {
        ...
        public DataSet GetGurdianEmail(string id)
        {
            connectionDB.OpenConnection();
            string sqlQuery = "select GurPhone from Student where StudentId = '" + id + "'";
            connectionDB.ExecuteQueries(sqlQuery);
            dataSet = new DataSet();
            dataSet = connectionDB.DataAdapter();
            connectionDB.CloseConnection();
        ...
        public int AddStudent(StudentDTO studentDTO)
        {
            ...
            catch (SqlException ex)
            {
                if (ex.Number == 2627)
                {
                    ...
                    connectionDB.CloseConnection();
                    MessageBox.Show("Student with same serial number already exits. Try new name for class", "Error..");
                    return 0;
