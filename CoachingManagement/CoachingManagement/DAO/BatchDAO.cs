    ...
    class BatchDAO
    {
        ...
        public void CreateBatch(BatchDTO batchDTO, ClassDTO classDTO)
        {
            try
            {
                ...
                connectionDB.ExecuteQueries(sqlQuery);
                connectionDB.CloseConnection();
                MessageBox.Show("Batch added successfully to the database", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            connectionDB.OpenConnection();
            //string sqlQuery = "update Batch set NumberOfStd = isnull((select count(*) from Student where (ClassName like '" + classname + "') and (BatchName like '" + batchname + "') group by ClassName,BatchName),0), AvailableSeat=(select ((MaxBatchStd) - (NumberOfStd)) from Batch where ClassName like '" + classname + "' and BatchName like '" + batchname + "') where ClassName like '" + classname + "' and BatchName like '" + batchname + "'";
        ...
        public void UpdateBatch(string classname,string batchname,string classcode,string tmpclass,string tmpbatch,string max, string num, string avil)
        {
            int available = Convert.ToInt32(max) - Convert.ToInt32(num);
                try
                {
                    ...
                    connectionDB.OpenConnection();
                    string sqlQuery3 = "update Batch set BatchName='" + batchname + "', MaxBatchStd = '" + max + "', AvailableSeat = '" + available + "'  where BatchName = '" + tmpbatch + "' and ClassName = '" + classname + "'";
                ...
                catch (SqlException ex)
                {
                    ...
                    {
                        connectionDB.CloseConnection();
                        MessageBox.Show("Already Batch: " + batchname + " exists under Class: " + classname + " \n Try a new name for batch", "Update Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
