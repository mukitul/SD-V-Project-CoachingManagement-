...
namespace CoachingManagement
{
    class StudentFeeDAO
    {
        ...
        {
            connectionDB.OpenConnection();
            string sqlQuery = "select StudentId as [Student Roll], ClassName as [Class Name], BatchName as [Batch Name],MonthlyFee as [Monthly Fee], FeeMonth as [Fee Of Month], FeeYear as [Year], FeeStatus as [Status], FeeId as [Payment Id] from StudentFee where StudentId='"+id+"'";
            connectionDB.ExecuteQueries(sqlQuery);
            dataSet = new DataSet();
            dataSet = connectionDB.DataAdapter();
            connectionDB.CloseConnection();
