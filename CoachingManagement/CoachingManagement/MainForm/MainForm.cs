...
namespace CoachingManagement
{
    public partial class MainForm : Form
    {
        ...
        {
            if (classbox.Text != "" && classcodebox.Text != "" && classfeebox.Text != "" )
            {
            ...
            {
                MessageBox.Show("Please Give All The Required Information", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void classupdatebtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("First Select A Row From Grid View To Update Entry.", "Warning..", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                if (classbox.Text != "" && classcodebox.Text != "" && classfeebox.Text != "")
        ...
        private void classdelbtn_Click(object sender, EventArgs e)
        {
            if (dataviewclass.SelectedRows.Count < 1)
            {
                MessageBox.Show("First Select A Row From Grid View To Delete Entry.","Warning..", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                if (classbox.Text != "" && classcodebox.Text != "" && classfeebox.Text != "")
                {
                    DialogResult result = MessageBox.Show("All the records of " + classbox.Text + " will be deleted.",
        ...
        {
            if(batchbox.Text != "" && classcombo.Text != "" && maxstd.Text != "")
            {
        ...
        private void batchupdatebtn_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                if (batchbox.Text != "" && classcombo.Text != "" && maxstd.Text != "")
                {
                    ...
                    {
                        MessageBox.Show("Sorry! \n You can not update seat number which is less than number of student of the batch.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
                else
                {
        ...
        private void studentaddbtn_Click(object sender, EventArgs e)
        {
            if(serialbox.Text != "" && studentnamebox.Text != "" && studentinstabox.Text != "" && studentclasscombo.Text != "" && studentbatchcombo.Text!="" && studentaddressbox.Text != "" && studentphonebox.Text != "" && gurdianphonebox.Text != "")
            {
                ...
                Boolean val = CheckSeatAvailability(stdbatch, stdclass);
                int a = studentDAO.AddStudent(new StudentDTO(serialno, stdname, stdinsta, stdclass, clscode, stdbatch, stdaddress, stdphone, gurphone, stdfee, stdyear));
                if (val == true && a==1)
                {
            ...
            {
                MessageBox.Show("Sorry! /n Please provide all the information ask here.","Error..");
            }
        ...
        {
            if(snamebox.Text != "" && scodebox.Text != "" && subclasscombo.Text !="" && subcodecombo.Text != "")
            {
        ...
        private void taddbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if(tserialbox.Text != "" && tnamebox.Text != "" && temailbox.Text != "" && taddressbox.Text != "" && teacherclasscombo.Text !="" && teachersubcombo.Text != "" && tbatchcombo.Text != "" && tsubcodebox.Text != "" && tsalarybox.Text !="")
                {
        ...
        {
            if(feerollbox.Text != "" && feemoneybox.Text != "" && feemonthcombo.Text != "" && feeyearbox.Text !="" && feestatuscombo.Text !="")
            {
            ...
            {
                MessageBox.Show("Please all the information correctly!");
            }
        ...
        private void LoadStudentFeeWithId(string id)
        {
            feedataview.DataSource = studentFeeDAO.GetStudentFeeWithId(id).Tables[0];
        }
        private void stdreceiptprintrollbox_TextChanged(object sender, EventArgs e)
        {
            if(stdreceiptprintrollbox.Text == "")
            {
                LoadStudentFee();
            }
            else
            {
                string id = stdreceiptprintrollbox.Text;
                LoadStudentFeeWithId(id);
            }
        }
        ...
        private void sendBtn_Click(object sender, EventArgs e)
        {
            ...
            client = new SmtpClient(textSetup.Text);
            client.Port = Convert.ToInt32(textPort.Text);
            client.EnableSsl = chkSSL.Checked;
            client.Credentials = login;
            msg = new MailMessage{ From = new MailAddress(textUserName.Text + textSetup.Text.Replace("smtp.", "@"), "mukitul076", Encoding.UTF8 ) };
        ...
        private void SendCompletedCallBack(object sender,AsyncCompletedEventArgs e)
        {
            ...
            {
                MessageBox.Show( "Email has been successfully sent!","Message", MessageBoxButtons.OK, MessageBoxIcon.Information);

            }
        }

        private void LoadGurdianEmail(string stdroll)
        {
            try
            {
                textTo.Text = "";
                textTo.Text = studentDAO.GetGurdianEmail(stdroll).Tables[0].Rows[0]["GurPhone"].ToString();
            }catch(Exception e)
            {
        ...
        private void emailstdroll_TextChanged(object sender, EventArgs e)
        {
            string stdroll = emailstdroll.Text;
            LoadGurdianEmail(stdroll);

        }

        private void LoadStudentResultWithId(string stdroll)
        {
            stdresview.DataSource = resultDAO.GetStudentResultWithId(stdroll).Tables[0];
        }
        private void resultstdrollbox_TextChanged(object sender, EventArgs e)
        {
            if (resultstdrollbox.Text == "")
            {
                LoadStudentResult();
            }
            else
            {
                string sid = resultstdrollbox.Text;
                LoadStudentResultWithId(sid);
            }
        }

        private void generateapplicationbtn_Click(object sender, EventArgs e)
        {
            ApplicationFormPrintPreview applicationFormPrintPreview = new ApplicationFormPrintPreview();
            applicationFormPrintPreview.ShowDialog();
        }
}  
